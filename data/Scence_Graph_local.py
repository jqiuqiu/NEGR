...
train_write_path='/home/qiujin/LiVLR/LiVLR-VideoQA-main/data/AGQA/AG/SGG/train/'
test_write_path='/home/qiujin/LiVLR/LiVLR-VideoQA-main/data/AGQA/AG/SGG/test/'
...
for img_id in img_ids:
    ...
    if mp4!=flag:
        if flag in train:
            train_write_file=open(os.path.join(train_write_path,flag+'_SGG.json'),'w')
            json.dump(train_sgg_info, train_write_file, ensure_ascii=False, sort_keys=True, indent=4)
            train_sgg_info={}
        elif flag in test:
            test_write_file=open(os.path.join(test_write_path,flag+'_SGG.json'),'w')
